...
sudo docker run -d --rm --name jenkinsci-byznass -u root -p 8080:8080 -p 50000:50000 -v "$jenkinsDataFolder"/data:/var/jenkins_home -v /var/run/docker.sock:/var/run/docker.sock jen
kinsci/byznass
